...
class CEOController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($date != 'custom') {
                ...
                $this->startDate = isset($request->start_date) ? $request->start_date : Carbon::now()->startOfDay();
                $this->endDate = isset($request->end_date) ? $request->end_date : Carbon::now()->endOfDay();

                $this->lastStartDate = isset($request->start_date) ? $request->start_date : Carbon::now()->subYear()->startOfDay();
                $this->lastEndDate = isset($request->end_date) ? $request->end_date : Carbon::now()->subYear()->endOfDay();

                $this->previousStartDate = isset($request->start_date) ? $request->start_date : Carbon::now()->startOfDay();
                $this->previousEndDate = isset($request->end_date) ? $request->end_date : Carbon::now()->endOfDay();
            }
    ...
    {
        $new_total_order_count = $new_total_revenue = $old_total_order_count = $old_total_revenue = $failed_order_repeat_purchase = 0;
        $new_order_course_category = $old_order_course_category = $new_order_previous = $old_order_previous = $failedOrderItemIDs = $data = [];
        ...
        $data['this_year_emi_enrollments_per'] = number_format((($data['this_year_emi_enrollments'] * 100) / ($data['this_year_emi_enrollments'] + $data['last_year_emi_enrollments'])), 2);
        $data['last_year_emi_enrollments_per'] = number_format((($data['last_year_emi_enrollments'] * 100) / ($data['this_year_emi_enrollments'] + $data['last_year_emi_enrollments'])), 2);
        $data['this_year_emi_revenue_per'] = number_format((($data['this_year_emi_revenue'] * 100) / ($data['this_year_emi_revenue'] + $data['last_year_emi_revenue'])), 2);
        $data['last_year_emi_revenue_per'] = number_format((($data['last_year_emi_revenue'] * 100) / ($data['this_year_emi_revenue'] + $data['last_year_emi_revenue'])), 2);
        $data['total_emi_revenue_per'] = number_format(((($data['this_year_emi_revenue'] - $data['last_year_emi_revenue']) / $data['last_year_emi_revenue']) * 100), 2);
        ...
        $data['total_revenue']['sum'] = number_format(($new_total_revenue + $old_total_revenue), 2);
        // $data['total_revenue']['per'] = min((($new_total_revenue - $old_total_revenue) / $old_total_revenue) * 100);
        // $data['total_revenue']['per'] = min(($new_total_revenue - $old_total_revenue) / $old_total_revenue * 100, 100);
        $data['total_revenue']['new_total_revenue'] = $new_total_revenue > 0 ? $new_total_revenue : 1;
        $data['total_revenue']['old_total_revenue'] = $old_total_revenue > 0 ? $old_total_revenue : 1;
        $data['total_revenue']['per'] = min(($data['total_revenue']['new_total_revenue'] - $data['total_revenue']['old_total_revenue']) / $data['total_revenue']['old_total_revenue'] * 100, 100);
    ...
    public function getCATData()
    {
        ...
        $new_today_cat_courses = DB::table('courses')
        ...
        // ->selectRaw('courses.slug as course_slug')
            ...
            ->whereBetween('orders.created_at', [$this->startDate, $this->endDate])
            ->whereRaw("FIND_IN_SET($categories->id, courses.category_ids)")
            ->where('courses.status', '=', 'published')
            ->groupBy('course_id')
            ->orderBy('courses.sort_order')
            ->get();

        $new_lastDay_cat_courses = DB::table('courses')
            ->selectRaw('courses.id as course_id,courses.name as course_name,courses.slug as course_slug,COUNT(orders.id) as count,SUM(orders.total) as total_amount')
        // ->selectRaw('courses.slug as course_slug')
            ->join('order_items', 'order_items.course_id', 'courses.id')
            ->join('orders', 'orders.id', 'order_items.order_id')
        ...
        foreach ($new_today_cat_courses as $course) {
        ...
        foreach ($new_lastDay_cat_courses as $course) {
        ...
        $old_today_cat_courses = DB::table('courses')
        ...
        $old_lastDay_cat_courses = DB::table('courses')
            ->selectRaw('courses.id as course_id,courses.name as course_name,courses.slug as course_slug,COUNT(old_orders.id) as count,SUM(old_orders.total_revenue) as total_amount')
            ->join('old_orders', 'old_orders.name', 'courses.name')
            ->where('courses.show_on_menu', '=', 'yes')
        ...
        foreach ($old_today_cat_courses as $course) {
            $old_today_count += $course->count;
            $old_today_revenue += $course->total_amount;
            $old_today['enrollment'][$course->course_name] = $course->count;
            $old_today['total_revenue'][$course->course_name] = $course->total_amount;
        }

        foreach ($old_lastDay_cat_courses as $course) {
            $old_lastDay_count += $course->count;
            $old_lastDay_revenue += $course->total_amount;
            $old_lastDay['enrollment'][$course->course_name] = $course->count;
            $old_lastDay['total_revenue'][$course->course_name] = $course->total_amount;
        }
